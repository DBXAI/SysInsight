...
vector<BitCastOperator *>
    verifiedStructTypeList; // vec to record the cast the turn Type
                            // %struct.redisServer into Type
                            // %struct.redisServer.xxx

vector<Instruction *>
    visitedLoadStoreInstList; // record visited Load/Store Inst.
vector<User *>
    visitedStructGVCases; // if a User is matched with a structual GV, we don't
                          // need to revisit it for other GVs.

vector<string> CommonLibFunctions;

vector<pair<pair<Type *, vector<int>>,
            Value *>>
    GEPTypeOffsetInstList; // record ALL GEPinstruction to make the anaylze
                           // field-sensitive. A little stupid but work.
...
/// Fetch the instrcution who use the result a GEP inst. who fetches the type
/// "Type*" with offsets "vector<int>"
Value *FetchValue4FurtherFollow(Type *type, vector<int> *indice,
                                Value *get_inst) {
  // pair<pair<Type *, vector<int>>, Value*> * GEPInfo){
  Value *match_value = nullptr;
  for (vector<pair<pair<Type *, vector<int>>, Value *>>::iterator i =
           GEPTypeOffsetInstList.begin();
       i != GEPTypeOffsetInstList.end(); i++) {
    if (i->first.first != type) // Type* not match
      continue;
    if (i->first.second != *indice) // offset not match
      continue;
    if (i->second == get_inst) // this gep_ins has been recorded before, by
                               // using the "addr" of store_ins
      continue;
...
vector<pair<CallBase *, Function *> *> getCallerAndCallInst(Function *f) {
  ...
  if (!f)
    llvm::outs()
        << "ERROR! Argument of `getCallerAndCallInst` should not be nullptr.\n";
  ...
  for (auto IT = bf_begin(CG), EI = bf_end(CG); IT != EI; IT++) {
    ...
    for (CallGraphNode::iterator calledNode_IT = IT->begin();
         calledNode_IT != IT->end(); calledNode_IT++) {
      ...
      if (CallBase *cb = llvm::dyn_cast<CallBase>(calledNode_IT->first)) {
        ...
        if (calledNode_IT->second->getFunction() &&
            calledNode_IT->second->getFunction() == f)

          res.push_back(
              new pair<CallBase *, Function *>(cb, IT->getFunction()));
      } else {

        llvm::outs() << "STRAGE. NOT a CallBase for the calledNode_IT->first"
                     << getClassType(calledNode_IT->first) << "\n";
      }
    }
  }
  return res;
...
void printCallers(vector<pair<CallBase *, Function *> *> a, uint level) {
  ...
  for (vector<pair<CallBase *, Function *> *>::iterator i = a.begin();
       i != a.end(); i++) {
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                               << "["
                               << getOriginalName((*i)->second->getName().str())
                               << "] ");
...
vector<string> splitWithTag(string str, string tag) {
  ...
  while (str.find(tag) != string::npos) {
    ...
    str = str.substr(str.find(tag) + tag.length(),
                     str.length() - str.find(tag) - tag.length());
  }
  names.push_back(str);
  return names;
}

bool readConfigVariableNames(
    std::string var_file,
  ...
  while (getline(fin, buf_str)) {
    ...
    if (isDynConfig == "Yes" || isDynConfig == "user" ||
        isDynConfig == "superuser" || isDynConfig == "backend" ||
        isDynConfig == "superuser-backend" || isDynConfig == "sighup") {
      config_name->isDynamicConfigurable = true;
    } else if (isDynConfig == "No" || isDynConfig == "internal" ||
  ...
  MY_DEBUG(
      _DEBUG_LEVEL, llvm::outs()
                        << "\n\n\n******************Output read in config name "
                           "info:******************\n\n\n\n";
...
string getStructTypeStrFromPrintAPI(Type *cur_type) {
  ...
  if (type_name.find("struct") == string::npos &&
      type_name.find("class") == string::npos)
    return "";
...
vector<unsigned> getIndexFromGEPO(GEPOperator *gepo) {
  /*
      This situation happends when handle on a array with index, like "tour[i] =
     tour[j];" in geqo_recombination.c:56; %42 = getelementptr i32, i32* %39,
     i64 %41 or pointer offset, like, "OldestVisibleMXactId =
     OldestMemberMXactId + MaxOldestSlot;" in multixact.c:1857 in postgresql,
     where "OldestMemberMXactId" and "OldestVisibleMXactId" both are pointer of
     MultiXactId, a typedef of uint32 finally. %80 = getelementptr i32, i32*
     %75, i64 %79 Since handling of config variables in structures seems not
     like this, so we don't consider it.
  */
  vector<unsigned> vec;
...
string getOriginalName(string manglingName) {
  ...
  int status;
  char *realname;
  realname = abi::__cxa_demangle(const_cast<char *>(manglingName.data()), 0, 0,
                                 &status);
  if (!realname)
    return manglingName;
  name = realname;
...
bool handleDIType(DIType *di_type,
                  ...
                  vector<unsigned> offsets, unsigned level) {
  ...
  if (config_names_idx != ERR_OORANGE && level != ERR_OORANGE) {
    MY_DEBUG(_REDUNDENCY_LEVEL,
             llvm::outs() << "FullName: "
                          << config_names[config_names_idx]->getNameAsString()
                          << "\n");
    if (level < config_names[config_names_idx]->StructName.size())
      MY_DEBUG(_REDUNDENCY_LEVEL,
               llvm::outs() << "Current_name "
                            << config_names[config_names_idx]->StructName[level]
                            << "\n");
  ...
  if (DIBasicType *basic_type = dyn_cast<DIBasicType>(di_type)) {
    if (config_names_idx != ERR_OORANGE &&
        config_names[config_names_idx]->VarType != SINGLE) {
      MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs() << "Comes here 3\n");
      ret_flag = true;
      struct GlobalVariableInfo *gv_info =
          new GlobalVariableInfo(config_names[config_names_idx], gv, offsets);
      gv_info_list.push_back(gv_info);
    } else {
      MY_DEBUG(_REDUNDENCY_LEVEL,
               llvm::outs() << "basic type gv name: " << gv->getName() << " -> "
                            << getOriginalName(gv->getName()) << "\n");
      for (unsigned i = 0; i < config_names.size(); i++) {
        if (config_names[i]->VarType == STRUCT ||
            config_names[i]->VarType == FIELD)
          continue;

        if (getOriginalName(gv->getName().str()) ==
            config_names[i]->SingleName) {
          ret_flag = true;
          struct GlobalVariableInfo *gv_info =
              new GlobalVariableInfo(config_names[i], gv, offsets);
          gv_info_list.push_back(gv_info);
        }
      }
    }
    ...
    if (derived_type->getTag() == llvm::dwarf::DW_TAG_member) {
      MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs()
                                      << "In DIDerivedType - member tag\n");

      if (config_names_idx == ERR_OORANGE || level == ERR_OORANGE)
        return false;

      MY_DEBUG(_REDUNDENCY_LEVEL,
               llvm::outs() << "Name : " << derived_type->getName() << "\n");
      MY_DEBUG(_REDUNDENCY_LEVEL,
               llvm::outs() << "level : " << level << " size : "
                            << config_names[config_names_idx]->StructName.size()
                            << "\n");
      ...
      if ((config_names[config_names_idx]->VarType == STRUCT &&
          ...
          (config_names[config_names_idx]->VarType == CLASS &&
           derived_type->getName().str() ==
               config_names[config_names_idx]->ClassName[level])) {
        ...
        if ((config_names[config_names_idx]->VarType == STRUCT &&
          ...
          return handleDIType(di_type, config_names, gv, gv_info_list,
                              config_names_idx, offsets, level);
        } else {
          MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs() << "Comes here 3\n");
          ret_flag = true;
          struct GlobalVariableInfo *gv_info = new GlobalVariableInfo(
              config_names[config_names_idx], gv, offsets);
          gv_info_list.push_back(gv_info);
        }
      }
    }
             ...
             derived_type->getTag() ==
                 llvm::dwarf::DW_TAG_restrict_type || // seems not useful, but
                                                      // we still consider it.
             derived_type->getTag() ==
                 llvm::dwarf::DW_TAG_atomic_type || // seems not useful, but we
                                                    // still consider it.
             derived_type->getTag() == llvm::dwarf::DW_TAG_volatile_type) {
      MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs()
                                      << "In DIDerivedType - other tag\n");
      DIType *type = derived_type->getBaseType();
      return handleDIType(type, config_names, gv, gv_info_list,
                          config_names_idx, offsets, level);
    } else {
      MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "Unhandled Tag in DIDerivedType: "
                                          << derived_type->getTag() << "\n");
    }
  }
    ...
    if (compose_type->getTag() == llvm::dwarf::DW_TAG_structure_type) {
      MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs()
                                      << "struct type compose_type name: "
                                      << compose_type->getName() << "\n");
      if (config_names_idx == ERR_OORANGE) {
        for (unsigned i = 0; i < config_names.size(); i++) {
          ...
          if (level == 0 && getOriginalName(compose_type->getName().str()) !=
                                config_names[i]->StructName[level])
            continue;

          unsigned offset = 0;
          MY_DEBUG(_REDUNDENCY_LEVEL,
                   llvm::outs() << "DW_TAG_structure_type elements size: "
                                << compose_type->getElements().size() << "\n");
          for (auto ite = compose_type->getElements().begin();
               ite != compose_type->getElements().end(); ite++, offset++) {
            DINode *di_node = *ite;
            MY_DEBUG(_REDUNDENCY_LEVEL,
                     llvm::outs() << "Here we iterate the elements\n");
            MY_DEBUG(_REDUNDENCY_LEVEL, di_node->print(llvm::outs()));
            MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs() << "\n");
            MY_DEBUG(_REDUNDENCY_LEVEL,
                     llvm::outs() << "current offset = " << offset << "\n");

            offsets.push_back(offset);
            ret_flag = true;

            if (isa<DIType>(di_node)) {
              handleDIType(dyn_cast<DIType>(di_node), config_names, gv,
                           gv_info_list, i, offsets, level + 1);
            }
            offsets.pop_back();
          }
        }
        ...
        for (auto ite = compose_type->getElements().begin();
          ...
          if (isa<DIType>(di_node)) {
            handleDIType(dyn_cast<DIType>(di_node), config_names, gv,
                         gv_info_list, config_names_idx, offsets, level + 1);
          }
          offsets.pop_back();
        }
      }
      ...
      for (unsigned i = 0; i < config_names.size(); i++) {
        ...
        // MY_DEBUG( _DEBUG_LEVEL, llvm::outs()<<"config_name:
        // "<<config_name.ClassName<<"::"<<config_name.FieldName<<"\n");
        if (getOriginalName(compose_type->getName().str()) !=
            config_names[i]->ClassName[level])
          continue;

        unsigned offset = 0;
        MY_DEBUG(_REDUNDENCY_LEVEL,
                 llvm::outs() << "DW_TAG_class_type elements size: "
                              << compose_type->getElements().size() << "\n");
        for (auto ite = compose_type->getElements().begin(),
                  e = compose_type->getElements().end();
             ite != e; ite++, offset++) {
          DINode *di_node = *ite;
          MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs()
                                          << "Here we iterate the elements\n");
      ...
      MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs()
                                      << "enum type compose_type name: "
                                      << compose_type->getName() << "\n");
      DIType *base_type = compose_type->getBaseType();
      handleDIType(base_type, config_names, gv, gv_info_list, config_names_idx,
                   offsets, level);
 ...
 *        CORE: struct GlobalVariableInfo* gv_info = new
 GlobalVariableInfo(config_names[config_names_idx], gv, offsets);
                gv_info_list.push_back(gv_info);
 *
 * @param config_names
 * @param gv
 ...
 * @return false
 */
bool findConfigVariable(
    std::vector<struct ConfigVariableNameInfo *> config_names,
    GlobalVariable *gv,
    std::vector<struct GlobalVariableInfo *> &gv_info_list) {
  MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs()
                                  << "Find whether it is a Config Variable.\n");
  MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs() << "GlobalVariable:\n\t");
  MY_DEBUG(_REDUNDENCY_LEVEL, gv->print(llvm::outs()));
  MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs() << "\n");

  // Usually DIGlobalVariableExpression only has 2 elements: a DIGlobalVariable
  // and a DIExpression
  ...
  if (handleDIType(di_gv->getType(), config_names, gv, gv_info_list,
                   ERR_OORANGE, offsets, 0))
    return true;
...
void collectBFSReachableBB(vector<BasicBlock *> &children, BasicBlock *bb) {
  ...
  while (!Q.empty()) {
    ...
    for (auto it = succ_begin(block); it != succ_end(block); ++it) {
      ...
      // to find if `succ` already in `children` (i.e., detect loop with 4-INF
      // nodes)
      if (std::find(children.begin(), children.end(), succ) != children.end())
        searchFlag = true;
...
void expandToAllReachableBB(vector<BasicBlock *> &childrens) {
  if (childrens.size() != 2 || childrens[0] == childrens[1]) {
    llvm::outs() << "[ERROR] getOperand(0)/(1), num_op=" << childrens.size()
                 << " (expected: 2), or something wrong here.\n";
    return;
  }
  queue<BasicBlock *> Q;
  Q.push(childrens[0]);
  ...
  while (!Q.empty()) {
    ...
    for (auto it = succ_begin(block); it != succ_end(block); ++it) {
      ...
      if (std::find(childrens.begin(), childrens.end(), succ) !=
          childrens.end())
        searchFlag = true;
      if (searchFlag == false)
        Q.push(succ);
 ...
 */
void traceUser(Value *cur_value, struct FuncInfo *func_info,
               struct InstInfo *prev_inst_info, uint level) {
  MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
  MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "[== " << __func__ << " ==]\n");
  /**********************************************************************************************
   **                                                ____ llvm::GlobalValue ____
   *llvm::GlobalObject ___ llvm:Function
   **                                               /
   **                            ____ llvm::Constant
   **                           /
  ...
  if (UserVec.size() == 0) {
    ...
    MY_DEBUG(_WARNING_LEVEL, llvm::outs()
                                 << "[Ret] Failed to find any user of ");
  ...
  for (auto i = UserVec.rbegin(), e = UserVec.rend(); i != e; i++) {
    ...
    if (isa<Instruction>(cur_user)) {
      MY_DEBUG(_WARNING_LEVEL, llvm::outs() << "\t");
      MY_DEBUG(_WARNING_LEVEL,
               llvm::outs()
                   << getSrcLoc(dyn_cast<Instruction>(cur_user)).toString()
                   << "\n");
    } else {
      MY_DEBUG(_WARNING_LEVEL, llvm::outs() << "\n");
    }
    ...
    if (Instruction *cur_inst = dyn_cast<Instruction>(cur_user)) {
      if (prev_inst_info && cur_inst == prev_inst_info->InstPtr) {
        ...
        MY_DEBUG(
            _WARNING_LEVEL,
            llvm::outs()
                << "[Continue to next user] `Previous ins` == `Current ins`. "
                   "Mostly because tracing user of *addr* of a storeIns: ["
                << getClassType(cur_inst) << "]\n");
        continue;
      }
      ...
      if (prev_inst_info &&
          comesBefore(inst_info->InstPtr, prev_inst_info->InstPtr)) {
        MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
        MY_DEBUG(_WARNING_LEVEL,
                 llvm::outs()
                     << "[Continue to next user] current instruction is before "
                        "the previous one within same function:"
                     << getClassType(cur_inst) << "]\n");
        MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
      ...
      /// NOTE: If the user is a continous StoreInst with same StoreAddr, we
      /// don't follow it.
      if (prev_inst_info && isa<StoreInst>(prev_inst_info->InstPtr) &&
        ...
        MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
        MY_DEBUG(_WARNING_LEVEL, llvm::outs()
                                     << "[Continue to next user] Don't trace "
                                        "continuous StoreInst.\n");
        continue;
      }

      /// NOTE: record visitedLSInst, since they are only one input and one
      /// output.
      if (isa<StoreInst>(inst_info->InstPtr) ||
          isa<LoadInst>(inst_info->InstPtr)) {
        if (std::find(visitedLoadStoreInstList.begin(),
                      visitedLoadStoreInstList.end(),
                      inst_info->InstPtr) != visitedLoadStoreInstList.end()) {
          MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
          MY_DEBUG(_WARNING_LEVEL, llvm::outs()
                                       << "[Continue to next user] Don't trace "
                                          "visited Load/Store Inst.\n");
          continue;
        }
      }
      ...
      if (prev_inst_info == nullptr) {
        ...
        if (findVisitedInstruction(inst_info, prev_inst_info)) {
          MY_DEBUG(_WARNING_LEVEL,
                   llvm::outs() << "This should not happen. Check me at line "
                                << __LINE__ << "\n");
          continue;
        }
      ...
      if (CallBase *call = dyn_cast<CallBase>(cur_inst)) {
        ...
        if (func == nullptr)
          continue;
        /// IGNORE: Debug-generated intrinsic functions, e.g.
        /// llvm.dbg.declare().
        if (func->isIntrinsic())
          continue;
        ...
        MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "Output: current Arg Index is : "
                                            << arg_index << "\n");
        struct FuncInfo *new_func_info = new FuncInfo(
            func, func_name, arg_index, inst_info->InstLoc.toString());
        unsigned index =
            isFuncInfoRecorded(new_func_info, func_info->InsideFuncInfoList);
        ...
        /// IGNORE: lib function.
        if (std::find(CommonLibFunctions.begin(), CommonLibFunctions.end(),
                      func_name) != CommonLibFunctions.end()) {
          MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
          MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                     << "[PASS] Calling to lib-function, check "
                                        "tainter.cpp `CommonLibFunctions`:"
                                     << func_name << "\n");
          continue;
        }

        /// IGNORE: If this Function has variable arguments' number, maybe we
        /// don't need to follow.
        if (func_info->Ptr->isVarArg()) {
          MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
          MY_DEBUG(_ERROR_LEVEL,
                   llvm::outs()
                       << "[ERROR, PASS] (but strange): Current call Arg Index "
                          "is larger than Function Arguments: "
                       << func_name << "\n");
          continue;
        }

        if (func_info->hasInsideDataFlowInfluence == UNDEFINE)
          // in traceFunction, we use `iterateAndCheck` to determine if there is
          // `hasInsideDataFlowInfluence` outisde to this func.
          traceFunction(new_func_info, level);

        // So if there is no `hasInsideDataFlowInfluence` or no ret val, do not
        // trace return value
        if (new_func_info->Ptr->getReturnType()->isVoidTy() ||
            new_func_info->hasInsideDataFlowInfluence == NO) {
          MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
          MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                     << "[PASS] return is Void, OR, no data "
        ...
        ////////////////////////////
        if (GetElementPtrInst *gep_inst =
                dyn_cast<GetElementPtrInst>(store_addr)) {

          MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
          MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                     << "This StoreInst stores to a address "
                                        "obtained by GetElementPtrInst, ");

          struct SrcLoc srcloc = getSrcLoc(gep_inst);

          if (srcloc.filenameHasString("include/c++") ||
              srcloc.dirHasString("include/c++")) {
            MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                       << "but from CXX library, IGNORE.");
          } else if (!gep_inst->hasIndices() ||
                     gep_inst->getNumOperands() < 3 ||
                     !gep_inst->hasAllConstantIndices()) {
            /** Good Example:
             ** %field_i = getelementptr inbounds %struct.TTT, %struct.TTT* %6,
             *i32 0, i32 8
             **                                               |       0        |
             *1  |  2  |   */
            MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
            MY_DEBUG(_DEBUG_LEVEL,
                     llvm::outs()
                         << "but num_op = " << gep_inst->getNumOperands()
                         << " (TODO) or not all indices are constant\n");
            MY_DEBUG(_DEBUG_LEVEL, srcloc.print(1));
          } else {
            MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
            MY_DEBUG(_DEBUG_LEVEL,
                     llvm::outs() << "Analyzing this GetElementPtrInst.\n");
            ...
            bool ignore = false;
            for (int i = 1; i < num; i++) {
              if (ConstantInt *second_offset =
                      dyn_cast<ConstantInt>(gep_inst->getOperand(i))) {
            ...
            if (ignore) {
              MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
              MY_DEBUG(_DEBUG_LEVEL,
                       llvm::outs() << "[NOTE] Ignore this GetElementPtrInst, "
                                       "some operand is not ConstantInt\n");
              MY_DEBUG(_DEBUG_LEVEL, gep_inst->print(llvm::outs()));
              MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "\n");
              MY_DEBUG(_DEBUG_LEVEL, srcloc.print(1));
            } else {
              // make this gep_inst into a gep_inst_info, and add this
              // gep_inst_info to the Successors/Predecessor list
              MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
              MY_DEBUG(_DEBUG_LEVEL,
                       llvm::outs()
                           << "[NOTE] We can handle this GetElementPtrInst.\n");
              struct SrcLoc srcloc = getSrcLoc(gep_inst);
               ...
               * This step is NOT accurate with an assumption that "same type
               * with same offset" is for conf only (if yes).
               *
              ...
              MY_DEBUG(_DEBUG_LEVEL,
                       llvm::outs()
                           << "[NOTE] Finding GetElementPtrInst with same "
                              "type, same offset, but not identical one.\n");
              ...
              if (matched_ins == nullptr) {
                ...
                MY_DEBUG(_DEBUG_LEVEL,
                         llvm::outs()
                             << "[NOT FOUND] software store conf to a xxx.yyy "
                                "but never use it? Strange!.\n");
              } else {
                /*
                 *    2. add matched_ins_info to list, then follow the
                 * matched_ins_info recursively.
                 */
                if (Instruction *matched_instruction =
                        dyn_cast<Instruction>(matched_ins)) {
                  /*
                   *    2.1.  make this matched_ins into a matched_ins_info, and
                   * add this matched_ins_info to the Successors/Predecessor
                   * list
                   */
                  struct SrcLoc srcloc = getSrcLoc(matched_instruction);
                  struct InstInfo *inst_info_matched_ins =
                      new InstInfo(matched_instruction, srcloc, false);
                  ...
                  MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
                  MY_DEBUG(_DEBUG_LEVEL,
                           llvm::outs() << "[FOUND] Follow this "
                                           "GetElementPtrInst recursively.\n");
                  traceUser(matched_instruction, func_info, inst_info_gep,
                            level + 1);
                } else {
                  MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
                  MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                             << "[WRONG] check me at line "
                                             << __LINE__ << "\n");
                }
              }
          ...
          /*
           *  define dso_local void @_Z11testAddressRii(i32* dereferenceable(4)
           * %a, i32 %b) #0 !dbg !935 { entry: \______ %a.addr = alloca i32*,
           * align 8                           when caller may give a reference,
           * we need be caution if tainting to it. %b.addr = alloca i32, align 4
           *       store i32* %a, i32** %a.addr, align 8  <-------------------
           * (3) %a.addr is stored with an address - %a (first argument) store
           * i32 %b, i32* %b.addr, align 4 %0 = load i32, i32* @CONFIG_VAR,
           * align 4, !dbg !942 %1 = load i32, i32* %b.addr, align 4, !dbg !943
           *       %or = or i32 %0, %1, !dbg !944
           *       %2 = load i32*, i32** %a.addr, align 8, !dbg !945 <--------
           * (2) %2 is the address stored in %a.addr store i32 %or, i32* %2,
           * align 4, !dbg !946 <--------------- (1) when store to %2
           */
        } else if (LoadInst *load_inst = dyn_cast<LoadInst>(store_addr)) {

          MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
          MY_DEBUG(_DEBUG_LEVEL,
                   llvm::outs() << "Pointer analysis: this StoreInst stores to "
                                   "a address loaded by LoadInst, follow the "
                                   "`addr` of it load from.\n");
          Value *load_addr = load_inst->getOperand(0);

          /// NOTE: `load_addr` is the address where maybe stored an address to
          /// (reference to) a caller's variable
          ///        but note that if you print the class of
          ///        `users_of_load_addr`, you may got a 'User::DerivedUser',
          ///        which is usually caused by instruction like: "a.addr =
          ///        alloca i32*, align 8"
          vector<User *> users_of_load_addr = getSequenceUsers(load_addr);

          /// NOTE: Rather than cast `users_of_load_addr` to instruction, we
          /// need to search in among the users of
          ///       this address to see: if there is a StoreInst ever store
          ///       something into this address. If yes, and this StoreInst
          ///       store the address (which is also a function arguments) to
          ///       it, then, the taint is out this function. We need to visit
          ///       callgraph to follow further in the caller.
          for (vector<User *>::iterator I_user = users_of_load_addr.begin();
               I_user != users_of_load_addr.end(); I_user++) {

            if (StoreInst *sb_store_to_this_addr =
                    dyn_cast<StoreInst>(*I_user)) {

              if (comesBefore(sb_store_to_this_addr, load_inst)) {
                Value *tobe_followed_reference =
                    sb_store_to_this_addr->getOperand(0);

                /// If hits at least one argument. If yes, visit callgraph to
                /// follow further in the caller.
                Function *thisFun = load_inst->getFunction();
                uint arg_index = 0;
                for (Function::arg_iterator arg_iter = thisFun->arg_begin();
                     arg_iter != thisFun->arg_end(); arg_iter++, arg_index++) {

                  if (arg_iter == tobe_followed_reference) {
                    /*
                     * Get users of the ReturnInst (which is a CallBase)
                     */
                    vector<pair<CallBase *, Function *> *> callers =
                        getCallerAndCallInst(thisFun);
                    printCallers(callers, level + 1);

                    /*
                     * For every users (CallBase) of the ReturnInst, trace their
                     * correspoding operand (argument).
                     */
                    for (vector<pair<CallBase *, Function *> *>::iterator i =
                             callers.begin();
                         i != callers.end(); i++) {
                      CallBase *caller_inst = (*i)->first;
                      struct SrcLoc srcloc = getSrcLoc(caller_inst);
                      struct InstInfo *inst_info_caller =
                          new InstInfo(caller_inst, srcloc);
                      ...
                      Value *tainted_tobe_followed =
                          caller_inst->getArgOperand(arg_index);
                      tainted_tobe_followed->print(llvm::outs());
                      llvm::outs() << "\n";

                      traceUser(tainted_tobe_followed, func_info,
                                inst_info_caller, level + 1);
                    }
                  }
                }
              } /// NOTE: NO `else` here, since if not comesBefore, the case can
                /// be passed happily.

            } /// NOTE: NO `else` here, since `handleUser(store_addr, gv_info,
              /// cur_inst_info, level+1);` will handle normal cases.
          }
        } else { // May be the class of 'store_addr' is 'User::DerivedUser'.
                 // Don't care
          MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
          MY_DEBUG(_DEBUG_LEVEL,
                   llvm::outs()
                       << "Pointer analysis: this StoreInst stores to a "
                          "address allocated by ["
                       << getClassType(store_addr)
                       << "], Currently, no need pointer analysis futher.\n");
        }
        ...
        ///////////////////////////////
        // Value *store_addr = store->getPointerOperand(); // e.g.  "store i32
        // %1, i32* @somevariable" traceUser(store_addr, func_info, inst_info,
        // level);    //                "@somevariable" should be traced
        /////////////////////////////////////////////////////////////////////////////////////
      } else if (isa<FenceInst>(cur_inst) || isa<AtomicCmpXchgInst>(cur_inst) ||
                 isa<AtomicRMWInst>(cur_inst)) {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL,
                 llvm::outs()
                     << "It is a FenceInst/AtomicCmpXchgInst/AtomicRMWInst, "
                        "STOP.\n");
      } else if (GetElementPtrInst *gep =
                     dyn_cast<GetElementPtrInst>(cur_inst)) {
        //////////////////////////
        /// Why this inst ?
        //////////////////////////

        /// whatever getNumOperands() = 3 or 2, we only trace the base
        /// situations.
        ...
        if (gep->getNumOperands() == 3 && (gep->getOperand(1) == cur_value ||
                                           gep->getOperand(2) == cur_value)) {
          inst_info->InfluenceLevel = WEAK;
          traceUser(gep, func_info, inst_info, level);
        }
      }
      /*
          Terminator Instruction:
              IndirectBrInst, BranchInst, ReturnInst, SwitchInst, InvokeInst?,
         CallBrInst?, ResumeInst, CatchSwitchInst, CatchReturnInst,
         CleanupReturnInst,
      */
      ...
      } else if (CatchSwitchInst *catch_switch =
                     dyn_cast<CatchSwitchInst>(cur_inst)) {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "It is a CatchSwitchInst.\n");
      } else if (CatchReturnInst *catch_return =
                     dyn_cast<CatchReturnInst>(cur_inst)) {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "It is a CatchReturnInst.\n");
      } else if (CleanupReturnInst *cleanup_ret =
                     dyn_cast<CleanupReturnInst>(cur_inst)) {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL,
                 llvm::outs() << "It is a CleanupReturnInst, stop tracing.\n");
      } else if (ReturnInst *ret_inst = dyn_cast<ReturnInst>(cur_inst)) {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL,
                 llvm::outs() << "It is a ReturnInst, stop tracing. We will "
                                 "determine if need to trace further from the "
                                 "ret of callcite of Caller Function \n");

        /// NOT_TODO: If the dataflow go to a return value, do we need to
        /// continue analyzing the usage of current Caller Function. WHY we need
        /// to do this? We have already made it in TraceFunction. What we need
        /// to do is to handle ReturnInst in `handleInstrcution`
      } else if (isa<AllocaInst>(cur_inst) || // generally don't need consider.
                 isa<CastInst>(cur_inst) || isa<ExtractValueInst>(cur_inst) ||
                 isa<FreezeInst>(cur_inst) || isa<LoadInst>(cur_inst) ||
                 isa<UnaryOperator>(cur_inst) ||
                 // isa<VAArgInst>(cur_inst) || // not usable at present.
                 isa<BinaryOperator>(cur_inst) || isa<CmpInst>(cur_inst) ||
                 isa<InsertValueInst>(cur_inst) || isa<PHINode>(cur_inst) ||
                 isa<SelectInst>(cur_inst) || isa<FreezeInst>(cur_inst) ||
                 isa<ShuffleVectorInst>(cur_inst) ||
                 isa<ExtractElementInst>(cur_inst) ||
                 isa<InsertElementInst>(cur_inst) ||
                 isa<FuncletPadInst>(cur_inst) ||
                 isa<LandingPadInst>(cur_inst) || isa<BitCastInst>(cur_inst) ||
                 isa<TruncInst>(cur_inst)) {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                   << "It is a " << cur_inst->getOpcodeName()
                                   << " Instruction.\n");
        // go further
        ...
        MY_DEBUG(_DEBUG_LEVEL,
                 llvm::outs()
                     << "Unknown type of instruction, but still trace it.\n");
        traceUser(cur_inst, func_info, inst_info, level);
      }
    }
      ...
      llvm::outs() << "[WARNING] Unhandled User Situation in traceUser: user "
                      "is a ConstantExpr\n";
      continue;
    }

    else if (ConstantAggregate *constant_struct =
                 dyn_cast<ConstantAggregate>(cur_value)) {
      llvm::outs() << "[WARNING] Unhandled User Situation in traceUser: user "
                      "is a ConstantAggregate\n";
      continue;
    }
      ...
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs()
                   << "[WARNING] Unhandled User Situation in traceUser:\n";
               cur_user->print(llvm::outs()); llvm::outs() << "\n";
               cur_user->getType()->print(llvm::outs()); llvm::outs() << "\n";
...
bool iterateAndCheck(struct InstInfo *inst_info) {
  if (isa<ReturnInst>(inst_info->InstPtr))
    return true;
  bool aa = false;
  for (auto it = inst_info->Successors.begin();
       it != inst_info->Successors.end(); ++it) {
...
/// OUTPUT: Return true if current argument has a dataflow to the return value
/// of this function.
void traceFunction(struct FuncInfo *func_info, uint level) {
  ...
  /*
   * If there exist at least one data flow from one of the function argument to
   * the return value. if YES, the return value will be continued to be followed
   * in the caller function Otherwise, won't.
   */
...
struct InstInfo *
MkNewInstInfoAndLinkOntoPrevInstInfo(struct Instruction *cur_inst,
...
void calcPHIedBB(BasicBlock *left, BasicBlock *right,
  ...
  for (auto &i : left_children) {
    if (std::find(right_children.begin(), right_children.end(), i) ==
        right_children.end())
...
bool taintPHINode(PHINode *phi_inst, DominatorTree *DT, BasicBlock *leftBB,
                  BasicBlock *rightBB) {
  ...
  if (left_dom < inBlockNum && left_dom > 0 ||
      right_dom < inBlockNum && right_dom > 0)
    return true;
  else
    return false;
}

// 1nd argument (BBsPhi) is the basic block set that need to be anaylzed for
// "PHI cases".
void handlePHINodesFromBBs(
    vector<BasicBlock *> &BBsPhi, // candidate BB where we find candidate PHI
    ...
    struct GlobalVariableInfo
        *gv_info, // if a candidate win, use it to trace further.
    struct InstInfo
        *cur_inst_info) // if a candidate win, use it to trace further.
{
  MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
  MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "[== " << __func__ << " ==]\n");

  // for iteration of all instruction
  for (vector<BasicBlock *>::iterator iB = BBsPhi.begin(); iB != BBsPhi.end();
       iB++) {

    // traverse each instruction, find PHINode.
    for (BasicBlock::iterator inst = (*iB)->begin(); inst != (*iB)->end();
         inst++) {
      ...
      if (PHINode *phi_inst = dyn_cast<PHINode>(inst)) {
        ...
        if (taintPHINode(phi_inst, DT, leftBB, rightBB)) {
          ...
          struct InstInfo *the_phi_ins = MkNewInstInfoAndLinkOntoPrevInstInfo(
              phi_inst, cur_inst_info, false);
          the_phi_ins->isDirect =
              false; // starting here, turn into indirect data-flow
          ...
          MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
          MY_DEBUG(_DEBUG_LEVEL,
                   llvm::outs() << the_phi_ins->InstLoc.toString() << "\n");
...
// 1st argument (BBs) is the tainted basic block set that the 3rd argument
// (cur_inst_info) produces. 3rd argument (cur_inst_info) is the branchInst.
void handleControFlowFromBBs(vector<BasicBlock *> &BBs,
  ...
  for (vector<BasicBlock *>::iterator iB = BBs.begin(); iB != BBs.end(); iB++) {
    ...
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL,
             llvm::outs() << "|  BasicBlock \"" << (*iB)->getName() << "\"\n");
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "+-----------------------------\n");
    ...
    for (BasicBlock::iterator inst = (*iB)->begin(); inst != (*iB)->end();
      ...
      if (StoreInst *store_inst = dyn_cast<StoreInst>(inst)) {
        ...
        if (call_inst->getCalledFunction() &&
            ...
            !isSubStr(call_inst->getCalledFunction()->getName().str(),
                      "llvm.dbg")) {
    ...
    if (store_ins_set.empty() && call_ins_set.empty()) {
      ...
      MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs() << "No storeInst or callBase in this basicBlock. "
                               "Control flow in this basic block stop here.\n");
      continue;
      ...
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs()
                   << "Only storeInst (no callBase) in this basicBlock. "
                      "Control flow continues from the stored address.\n");
#ifdef CONTROL_STORE
      for (vector<StoreInst *>::iterator i = store_ins_set.begin();
           i != store_ins_set.end(); i++) {
        struct InstInfo *the_store_ins =
            MkNewInstInfoAndLinkOntoPrevInstInfo(*i, cur_inst_info, false);
        handleInstruction((*i)->getValueOperand(), gv_info, the_store_ins,
                          level + 1);
      }
#else
      MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs() << "Def `CONTROL_STORE` to enable storeInst.\n");
#endif
    } else if (store_ins_set.empty() && !call_ins_set.empty()) {
      ...
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs()
                   << "Only callBase (no storeInst) in this basicBlock. We "
                      "take all calls as the influenced functions finally. And "
                      "control flow in this basic block stop here.\n");
      for (vector<CallBase *>::iterator i = call_ins_set.begin();
           i != call_ins_set.end(); i++) {
        cur_inst_info->addControllingFuncs((*i)->getCalledFunction());
        gv_info->InfluencedFuncList[gv_info->currentGVStartingFuncName]
            .push_back((*i)->getCalledFunction());
      }
    } else {
      ...
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs()
                   << "Both storeInst and callBase in this basicBlock. The "
                      "most complex situation here. Control flow continues "
                      "from the stored address and take all calls as the "
...
unsigned isFuncInfoRecorded(struct FuncInfo *_funcinfo,
                            vector<struct FuncInfo *> FuncInfoList) {
...
#undef DEBUG_calcTaintBBfromBr
  ...
  next_br_successors.insert(next_br_successors.end(),
                            cur_br_successors->begin(),
                            cur_br_successors->end());
  ...
  for (auto cur_br_successors_i : *cur_test_successors) {
    ...
    for (auto cur_br_successors_j : *cur_br_successors) {
      ...
      // ------- taint! --------
      // if `cur_br_successors_i` does not post-dominate all successors, it is
      // tainted. (it at least post-dominate itself)
      if (!PDT->dominates(&*cur_br_successors_i->begin(),
                          &*cur_br_successors_j->begin())) {

        if (std::find(TaintedBBs->begin(), TaintedBBs->end(),
                      cur_br_successors_i) == TaintedBBs->end())
          TaintedBBs->push_back(cur_br_successors_i);

        // for one step further in CFG.
        for (BasicBlock *succ_OF_cur_br_successors_i :
             successors(cur_br_successors_i)) {

          // prevent loop.
          if (std::find(cur_br_successors->begin(), cur_br_successors->end(),
                        succ_OF_cur_br_successors_i) ==
              cur_br_successors->end()) {
        ...
        // this loop is only to determine if `cur_br_successors_i` is tainted or
        // not, once hit, this layer of loop can over.
        break;
      }
  ...
  calcTaintBBfromBr(&next_test_successors, &next_br_successors, TaintedBBs,
                    PDT);
}

/**
 *  call in `handleUser`: handleInstruction(cur_value, gv_info, inst_info,
 * level);
 *                                              \___ isa<GlobalVariable> (maybe)
 */
void handleInstruction(
    Value *cur_value, // one of the user of `cur_inst_info->ptr`
    struct GlobalVariableInfo *gv_info,
    struct InstInfo
        *cur_inst_info, // the last inst before this user `cur_value`
    unsigned level) {
  MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
  MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "[== " << __func__ << " ==]\n");
  ...
  /**
   ** `CallBase` is the base class for all callable instructions (`InvokeInst`
   *and `CallInst`)
   ...
   **     void bar() {
   **         try {
   **             foo();                  ==InvokeInst==>   invoke void
   *@_Z3foov() to label %5 unwind label %6
   **         } catch (MyError err) {}                      5:
   **     }                                                   br label %18   ##
   *Normal branch, do nothing.
   **                                                       6:
   **                                                         %7 = landingpad {
   *i8*, i32 } ... ## error handling
   ** -----------------------------------------------------------------------------------------------------
   **/
  ///
  if (CallBase *call = dyn_cast<CallBase>(cur_inst)) {
     ...
     *ID id, ArrayRef< Type * > Tys = None)
     **         =?= llvm::Type::TypeID.FunctionTyID
     **         Function * llvm::Intrinsic::getDeclaration(LLVMContext &
     *Context, ID id, ArrayRef< Type * > Tys = None)
     **/
    if (func == nullptr)
      return;
    ...
    struct FuncInfo *func_info = new FuncInfo(
        func, func_name, arg_index, cur_inst_info->InstLoc.toString());
    ...
    if (index == ERR_OORANGE) // never recorded before
    {
      ...
      /**
       ** have recorded before, so this another time of calling. In this case,
       *use
       ** `gv_info->FuncInfoList[index]->CallLocList` to add one more element
       *(source location)
    ...
    if (std::find(CommonLibFunctions.begin(), CommonLibFunctions.end(),
                  func_info->FuncName) != CommonLibFunctions.end()) {
      MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
      MY_DEBUG(_WARNING_LEVEL, llvm::outs() << "It is a log function, stop. "
                                            << func_info->FuncName << ".\n");
      return;
    }
    ...
    {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_WARNING_LEVEL, llvm::outs() << "It is a lib function, need
    rule. " << func_info->FuncName << ".\n"); return;
    }
    */

    /// NOTE: If this Function has variable arguments' number, maybe we don't
    /// need to follow.
    ///       e.g., fun(x, y, ..) / fun(x, y).  Corner cases.
    if (func_info->Ptr->isVarArg()) {
      MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
      MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                 << "It is a function with variable arguments: "
                                 << func_info->FuncName << ".\n");
      return;
    }
    ...
    /// TODO: Do we need to consider Reference Passing in arguments of
    /// functions?
    if (func->getReturnType()->isVoidTy() ||
        func_info->hasInsideDataFlowInfluence == NO) {
      MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
      MY_DEBUG(_WARNING_LEVEL,
               llvm::outs() << "Don't need to trace after this function: "
                            << func_info->FuncName << ".\n");
      return;
    } else {
    ...
    // `call` is a llvm::Function, its parent class is llvm::GlobalValue, this
    // will branch into 'return'.
    /******************************************************************
     *   if(  call != gv_info->Ptr
    ...
    /// NOTE: if gv is the store addr operand, we don't trace this situation,
    /// since it is a destination.
    if (level != 0 && gv_info->NameInfo->VarType == SINGLE &&
        store_addr == gv_info->Ptr)
      return;
    if (level != 0 && (gv_info->NameInfo->VarType == STRUCT ||
                       gv_info->NameInfo->VarType == CLASS)) {
      if (GEPOperator *store_gepo = dyn_cast<GEPOperator>(store_addr)) {
        if (gv_info->GlobalVariableType != "" &&
            getStructTypeStrFromPrintAPI(store_gepo->getType()) ==
                gv_info->GlobalVariableType)
          return;
      }
    }
    ...
    if (GetElementPtrInst *gep_inst = dyn_cast<GetElementPtrInst>(store_addr)) {
      ...
      if (srcloc.filenameHasString("include/c++") ||
      ...
      } else if (!gep_inst->hasIndices() || gep_inst->getNumOperands() < 3 ||
                 !gep_inst->hasAllConstantIndices()) {
        /** Good Example:
         ** %field_i = getelementptr inbounds %struct.TTT, %struct.TTT* %6, i32
         *0, i32 8
         **                                               |       0        |  1
         *|  2  |   */
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL,
                 llvm::outs() << "but num_op = " << gep_inst->getNumOperands()
                              << " (TODO) or not all indices are constant\n");
        MY_DEBUG(_DEBUG_LEVEL, srcloc.print(1));
      } else {
        MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
        MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                   << "Analyzing this GetElementPtrInst.\n");
        ...
        if (ignore) {
          MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
          MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                     << "[NOTE] Ignore this GetElementPtrInst, "
                                        "some operand is not ConstantInt\n");
          MY_DEBUG(_DEBUG_LEVEL, gep_inst->print(llvm::outs()));
          MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "\n");
          MY_DEBUG(_DEBUG_LEVEL, srcloc.print(1));
        } else {
          // make this gep_inst into a gep_inst_info, and add this gep_inst_info
          // to the Successors/Predecessor list
          MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
          MY_DEBUG(_DEBUG_LEVEL,
                   llvm::outs()
                       << "[NOTE] We can handle this GetElementPtrInst.\n");
          ...
          /*
           * Find same type with same offset but not identical gep_inst
           * This step is NOT accurate with an assumption that "same type with
           * same offset" is for conf only (if yes).
           *
           *    1. iterate over all get_inst. find the matched one.
           */
          MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
          MY_DEBUG(_DEBUG_LEVEL,
                   llvm::outs()
                       << "[NOTE] Finding GetElementPtrInst with same type, "
                          "same offset, but not identical one.\n");
          Value *matched_ins = nullptr;
          matched_ins = FetchValue4FurtherFollow(type, &indices, gep_inst);
          if (matched_ins == nullptr) {
            MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
            MY_DEBUG(_DEBUG_LEVEL,
                     llvm::outs() << "[NOT FOUND] software store conf to a "
                                     "xxx.yyy but never use it? Strange!.\n");
          } else {
            /*
             *    2. add matched_ins_info to list, then follow the
             * matched_ins_info recursively.
            ...
            if (Instruction *matched_instruction =
                    dyn_cast<Instruction>(matched_ins)) {
              /*
               *    2.1.  make this matched_ins into a matched_ins_info, and add
               * this matched_ins_info to the Successors/Predecessor list
               */
              struct SrcLoc srcloc = getSrcLoc(matched_instruction);
              struct InstInfo *inst_info_matched_ins =
                  new InstInfo(matched_instruction, srcloc, false);
              ...
              MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                         << "[FOUND] Follow this "
                                            "GetElementPtrInst recursively.\n");
              handleUser(matched_instruction, gv_info, inst_info_gep,
                         level + 1);
            } else {
              MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
              MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "[WRONG] check me at line "
                                                  << __LINE__ << "\n");
            }
          }
          // printSequenceUsers(gep_inst->getOperand(0));
      ...
      /// NOTE: The case we handle here is like:
      /*
       *  define dso_local void @_Z11testAddressRii(i32* dereferenceable(4) %a,
       * i32 %b) #0 !dbg !935 { entry: \______ %a.addr = alloca i32*, align 8
       * when caller may give a reference, we need be caution if tainting to it.
       *       %b.addr = alloca i32, align 4
       *       store i32* %a, i32** %a.addr, align 8  <------------------- (3)
       * %a.addr is stored with an address - %a (first argument) store i32 %b,
       * i32* %b.addr, align 4 %0 = load i32, i32* @CONFIG_VAR, align 4, !dbg
       * !942 %1 = load i32, i32* %b.addr, align 4, !dbg !943 %or = or i32 %0,
       * %1, !dbg !944 %2 = load i32*, i32** %a.addr, align 8, !dbg !945
       * <-------- (2) %2 is the address stored in %a.addr store i32 %or, i32*
       * %2, align 4, !dbg !946 <--------------- (1) when store to %2
       */
    } else if (LoadInst *load_inst = dyn_cast<LoadInst>(store_addr)) {
      ...
      MY_DEBUG(
          _DEBUG_LEVEL,
          llvm::outs()
              << "Pointer analysis: this StoreInst stores to a address loaded "
                 "by LoadInst, follow the `addr` of it load from.\n");
      Value *load_addr = load_inst->getOperand(0);

      /// NOTE: `load_addr` is the address where maybe stored an address to
      /// (reference to) a caller's variable
      ///        but note that if you print the class of `users_of_load_addr`,
      ///        you may got a 'User::DerivedUser', which is usually caused by
      ///        instruction like: "a.addr = alloca i32*, align 8"
      vector<User *> users_of_load_addr = getSequenceUsers(load_addr);

      /// NOTE: Rather than cast `users_of_load_addr` to instruction, we need to
      /// search in among the users of
      ///       this address to see: if there is a StoreInst ever store
      ///       something into this address. If yes, and this StoreInst store
      ///       the address (which is also a function arguments) to it, then,
      ///       the taint is out this function. We need to visit callgraph to
      ///       follow further in the caller.
      for (vector<User *>::iterator I_user = users_of_load_addr.begin();
           I_user != users_of_load_addr.end(); I_user++) {
        ...
        if (StoreInst *sb_store_to_this_addr = dyn_cast<StoreInst>(*I_user)) {
          ...
          if (comesBefore(sb_store_to_this_addr, load_inst)) {
            ...
            /// If hits at least one argument. If yes, visit callgraph to follow
            ...
            for (Function::arg_iterator arg_iter = thisFun->arg_begin();
              ...
              if (arg_iter == tobe_followed_reference) {
                ...
                for (vector<pair<CallBase *, Function *> *>::iterator i =
                  ...
                  handleUser(tainted_tobe_followed, gv_info, inst_info_caller,
                             level + 1);
                }
              }
            }
          } /// NOTE: NO `else` here, since if not comesBefore, the case can be
            /// passed happily.

        } /// NOTE: NO `else` here, since `handleUser(store_addr, gv_info,
          /// cur_inst_info, level+1);` will handle normal cases.
      }
    } else { // May be the class of 'store_addr' is 'User::DerivedUser'. Don't
             // care
      MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs()
                   << "Pointer analysis: this StoreInst stores to a address "
                      "allocated by alloca (User::DerivedUser), so no need "
                      "pointer analysis futher.\n");
    }
    ...
    /*
        If GEP has 2 operands, and cur_value is the gep_base(the offset is often
       constant), we continue following the flow; if cur_value is the offset, we
       should not follow.

        If GEP has 3 operands, and cur_value is the gep_base, and the second
       operand is constant(often 0), we continue following; if cur_value is the
       third operand, we should not follow.
    */
    ...
    if (cur_inst_info->Predecessor == nullptr) {
      if (isMatchedGEPOperator(dyn_cast<GEPOperator>(gep), gv_info, level)) {
        MY_DEBUG(_WARNING_LEVEL,
                 llvm::outs() << "Found start point of config: "
                              << gv_info->NameInfo->getNameAsString() << "\n");
        visitedStructGVCases.push_back(cur_inst);

        /// NOTE: We found the gep instruction to get structural configuration
        /// option, so we trace it.
  ...
  */
    ...
    if (select_inst->getCondition() != nullptr &&
      ...
      // There may be argue further. Should we follow such case? I think yes.
      // because select is the most simple way to convert configuration variable
      // to other form of variable.
      //    e.g., open_flag = select,
      //    srv_unix_file_flush_method==SRV_UNIX_O_DSYNC? O_SYNC : xxx
      MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs()
                   << "[Control-Flow WARINING] following a selectInst, where "
                      "the pre-taint is the condition.\n");
      ...
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs()
                   << "following a selectInst, where the pre-taint is the "
                      "if-true/if-false value (this is data-flow).\n");
      handleUser(select_inst, gv_info, cur_inst_info, level + 1);
    }
  } else if (BranchInst *branch = dyn_cast<BranchInst>(cur_inst)) {
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    ...
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "Num of Successors: "
                                        << branch->getNumSuccessors() << "\n");

    if (branch->isUnconditional() || branch->getNumSuccessors() != 2) {
      MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
      MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                 << "It should be conditional Branch.");
      MY_DEBUG(_ERROR_LEVEL,
               llvm::outs()
                   << "Or it should not have more than 2 Successors.\n");
      return;
      ...
      if (leftBB && rightBB && rightBB->getParent() != leftBB->getParent()) {
        MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
        MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                   << "[ERROR] parent of the two children are "
                                      "not expected, RETERN.\n");
        return;
      }
...
#undef OLD_JUDGE_TAINT
...
#endif
      MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
      MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "exp.-to-all children size: "
                                          << childrens.size() << "\n");
...
#ifdef OLD_JUDGE_TAINT
      for (vector<BasicBlock *>::iterator iB = childrens.begin();
           iB != childrens.end(); iB++) {
        /*
         *** The core to determine if a BB is tainted -- the "post-dominance"
         **** A block Y is control dependent on block X if and only if Y post
         *dominates at least one but not all successors of X. TO A block Y is
         *dependent on block X if and only if Y post dominates not all
         *successors of X. When encounter a none-control-dependent node, stop
         *determining its children.
         *
      ...
      if (taintedBB.empty()) {
        ...
        MY_DEBUG(_ERROR_LEVEL,
                 llvm::outs()
                     << "[ERROR] VERY Strange case. "
                        "None-post-dominant-branches should NOT be empty.\n");
        return;
      }
      MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
      MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "tainted basic-blocks: "
                                          << taintedBB.size() << "\n");
      ...
      if (PHIedBB.empty()) {
        MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
        MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                   << "[NOTE] no PHIed nodes here, i.e., no "
                                      "auxillary data flow.\n");
      }
      MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
#ifdef PHIBB_UNION
...
#endif
      MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "PHI-ed basic-blocks: "
                                          << PHIedBB.size() << "\n");

      /// currently, we follow phi cases infinitely;
      /// TODO: add max limitation on the flow length.
      handlePHINodesFromBBs(PHIedBB, leftBB, rightBB, DT, level + 1, gv_info,
                            cur_inst_info);

      /// TODO: iterate cur_inst_info and its Predecessors to find whether there
      /// is used Values in taintedBB.
      ///       If not, just record diff_bbs as ControllingBBs in BranchInst as
      ///       currently done.
      ...
      MY_DEBUG(_ERROR_LEVEL,
               llvm::outs()
                   << "This Instruction marked as a ControllingInst 2.\n");
      cur_inst_info->isControllingInst = true;
      cur_inst_info->setControllingBBs(taintedBB);
    } else {
      MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
      MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "WTF is this?? check me in line "
                                          << __LINE__ << ".\n");
    }
  } else if (SwitchInst *switch_inst = dyn_cast<SwitchInst>(cur_inst)) {
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL,
             llvm::outs() << "It is a SwitchInst, with cases: "
                          << switch_inst->getNumCases() << ", and successors: "
                          << switch_inst->getNumSuccessors() << "\n");

    // calculated the set of BB should be tainted.
    vector<BasicBlock *> children, taintedBBs;
    PostDominatorTree *PDT =
        new PostDominatorTree(*cur_inst->getParent()->getParent());
    ...
    if (taintedBBs.empty()) {
      MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
      MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                 << "[ERROR] VERY Strange case. should be at "
                                    "least one element.\n");
      return;
    }
    MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
    ...
    MY_DEBUG(
        _DEBUG_LEVEL,
        llvm::outs()
            << "It is a FenceInst/AtomicCmpXchgInst/AtomicRMWInst, STOP.\n");
  } else if (ResumeInst *resume_inst = dyn_cast<ResumeInst>(cur_inst)) {
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "It is a ResumeInst.\n");
  } else if (CatchSwitchInst *catch_switch =
    ...
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                               << "It is a CleanupReturnInst, stop tracing.\n");
  } else if (ReturnInst *ret_inst = dyn_cast<ReturnInst>(cur_inst)) {
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL,
             llvm::outs() << "It is a ReturnInst, since we reach a ret, we "
                             "need to go to caller. Caller-CallInst pairs of ["
                          << getOriginalName(
                                 ret_inst->getFunction()->getName().str())
                          << "]:\n");
    ...
    vector<pair<CallBase *, Function *> *> callers =
        getCallerAndCallInst(thisfunc);
    endTime = clock();
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL,
             llvm::outs() << "traversed callgraph for ["
                          << (endTime - startTime) / CLOCKS_PER_SEC << "] secs."
                          << "\n";);
    printCallers(callers, level + 1);
     ...
     */
    for (vector<pair<CallBase *, Function *> *>::iterator i = callers.begin();
         i != callers.end(); i++) {
      CallBase *caller_inst = (*i)->first;
      struct SrcLoc srcloc = getSrcLoc(caller_inst);
      ...
      isa<CmpInst>(cur_inst) || isa<InsertValueInst>(cur_inst) ||
      isa<PHINode>(cur_inst) || isa<FreezeInst>(cur_inst) ||
      isa<ShuffleVectorInst>(cur_inst) || isa<ExtractElementInst>(cur_inst) ||
      isa<InsertElementInst>(cur_inst) ||
      // FuncletPadInst includes CatchPadInst, CleanupPadInst
      isa<FuncletPadInst>(cur_inst) || isa<LandingPadInst>(cur_inst)) {
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                               << "It is a " << cur_inst->getOpcodeName()
                               << " Instruction.\n");
    ...
    MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
    MY_DEBUG(_WARNING_LEVEL, llvm::outs() << "Unhandled Instruction, Opcode: "
                                          << cur_inst->getOpcodeName() << "\n");
...
bool reachTargetFunction(Function *base, Function *target,
...
bool reachTargetBasicBlock(BasicBlock *base, BasicBlock *target,
                           vector<BasicBlock *> &visitedBB) {
  for (BasicBlock *succ : successors(base)) {
    // if `succ` not in `visitedBB`, continue to next `succ` (because if in,
    // `succ` must has been explored)
...
bool comesBefore(Instruction *tar, Instruction *base) {
  ...
  if (base->getParent()->getParent() != tar->getParent()->getParent()) {
    vector<Function *> visitedFuncs;
    visitedFuncs.clear();
    // condition is True if there is a *forward* path from `base`'s function to
    // `tar`'s function
    if (reachTargetFunction(base->getParent()->getParent(),
                            tar->getParent()->getParent(), visitedFuncs)) {
      // in other words, there is not a *forward* path from `tar` to `base`, so
      // return false.
      return false;
    } else {
      return true;
  ...
  if (base->getParent() != tar->getParent() &&
    ...
    vector<BasicBlock *> visitedBB;
    visitedBB.clear();
    // condition is True if there is a *forward* path from `base`'s basic-block
    // to `tar`'s basic-block
  ...
  for (BasicBlock::iterator i = bb->begin(); i != bb->end(); i++) {
    Instruction *inst = &*i;
    if (inst == base) // we meet `base` first, the next `if` will fail later
                      // (because flag_meet = true), and finally return false
      flag_meet = true;
...
std::vector<User *> getSequenceUsers(Value *cur_value) {
  vector<User *> UserVec;
  UserVec.clear();
  for (Value::user_iterator i = cur_value->user_begin(),
                            e = cur_value->user_end();
       i != e; i++) {
...
vector<Use *> getSequenceUses_OnlyGEPins(Value *cur_value) {
  ...
  vector<Use *> UseVec;
  UseVec.clear();
  for (Value::use_iterator i = cur_value->use_begin(), e = cur_value->use_end();
       i != e; i++) {
    if (GetElementPtrInst *gep_ins = dyn_cast<GetElementPtrInst>(*i)) {
      ...
      MY_DEBUG(_DEBUG_LEVEL,
               llvm::outs() << "NOT gep_ins, but " << getClassType(*i) << "\n");
    }
  }
  return UseVec;
}
...
bool findVisitedInstruction(struct InstInfo *inst_info,
                            struct InstInfo *prev_inst_info) {
  /// if any of the two is nullptr, return true?
  if (!inst_info || !prev_inst_info) {
    MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                               << "inst_info is nullptr. Check me at line: "
                               << __LINE__ << "\n");
    return false;
  }
  ...
  while (temp) {
    if (inst_info->InstPtr == temp->InstPtr) {
      MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                 << "Find in reverse the previous Inst node "
                                 << cnt << "\n");
...
bool isMatchedGEPOperator(GEPOperator *gepo, struct GlobalVariableInfo *gv_info,
  ...
  /// NOTE: if this is the start point of structural GVs' gep instruction, check
  /// and follow.
  if (gv_info->NameInfo->VarType == STRUCT &&
      (getStructTypeStrFromPrintAPI(gepo->getPointerOperandType()) ==
           gv_info->GlobalVariableType || // directly match base with GVs.
       std::find(verifiedStructTypeList.begin(), verifiedStructTypeList.end(),
                 gepo->getPointerOperand()) !=
           verifiedStructTypeList.end()) // previously verified bitcast type
                                         // same to %struct.redisServer.
  ) {
    ...
    if (idx_vec.empty()) {
      ...
      MY_DEBUG(
          _WARNING_LEVEL,
          llvm::outs()
              << "Unexpected Situation: Error in get Indexs in GEPOperator.\n");
      return false;
    }
 ...
 **         gv  ~~  GlobalVariable*
 **/
void handleUser(Value *cur_value, struct GlobalVariableInfo *gv_info,
                struct InstInfo *prev_inst_info, unsigned level) {
  MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
  MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "[== " << __func__ << " ==]\n");
  ...
  if (!cur_value && level > 0)
     ...
     * ========== FIELD case ==========
     * Find same type with same offset but not identical gep_inst
     * This step is NOT accurate with an assumption that "same type with same
     * offset" is for conf only (if yes).
     *
     *    1. iterate over all get_inst. find the matched one.
     */
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL,
             llvm::outs() << "Entry point of non-global pointer parameter.\n");
    MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
    MY_DEBUG(_DEBUG_LEVEL,
             llvm::outs() << "[NOTE] Finding GetElementPtrInst with same type, "
                             "same offset, but not identical one.\n");
    Value *matched_ins = nullptr;
    vector<int> indices;
    ...
    matched_inss = FetchValue4FurtherFollow2(
        gv_info->NameInfo->getNameAsStringPrefix(), &indices);
    if (matched_inss.size() == 0) {
      MY_DEBUG(_DEBUG_LEVEL, printTabs(level + 1));
      MY_DEBUG(_DEBUG_LEVEL, llvm::outs()
                                 << "[WARNING] Not found use of "
                                 << gv_info->NameInfo->getNameAsString()
                                 << ". Check *-parameter.txt file\n");
      return;
    }
    for (auto matched_ins : matched_inss) {
      if (matched_ins == nullptr) {
        ...
        MY_DEBUG(
            _DEBUG_LEVEL,
            llvm::outs()
                << "[ERROR] current matched_ins is null. Next matched_ins.\n");
        continue;
      } else {
        /*
         *    2. add matched_ins_info to list, then follow the matched_ins_info
         * recursively.
         */
        if (Instruction *matched_instruction =
                dyn_cast<Instruction>(matched_ins)) {
          ...
          MY_DEBUG(
              _DEBUG_LEVEL,
              llvm::outs()
                  << "[FOUND] Follow this GetElementPtrInst recursively.\n");
  ...
  /**
   ** NOTE: If goto a global variable, we treat that gv's influencial zone is
   *included by current gv.
   **       So we record it, and stop visiting its Users.
   **/
  if (gv_info->Ptr && cur_value != gv_info->Ptr &&
      isa<GlobalVariable>(cur_value)) {
    MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Come to Global Variable: "
                                        << cur_value->getName() << ", stop\n");
    gv_info->InfluencedGVList.push_back(dyn_cast<GlobalVariable>(cur_value));
    // return;
  }

  /**
   * @brief for(Value::user_iterator i=cur_value->user_begin(),
   * e=cur_value->user_end(); i!=e; i++) UserVec.push_back(*i);
   */
  vector<User *> UserVec = getSequenceUsers(cur_value);
  unsigned user_ite_cnt = 0;
  ...
  for (auto i = UserVec.rbegin(), e = UserVec.rend(); i != e; i++) {
    ...
    /// If cur_user is a struct GV User, and matched with previous GVs, we don't
    /// waste time to handle it once more.
    if (std::find(visitedStructGVCases.begin(), visitedStructGVCases.end(),
                  cur_user) != visitedStructGVCases.end())
      continue;
    ...
    if (Instruction *cur_inst = dyn_cast<Instruction>(cur_user)) {
      ...
      if (prev_inst_info && cur_inst == prev_inst_info->InstPtr) {
        MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
        MY_DEBUG(_WARNING_LEVEL,
                 llvm::outs() << "`Previous ins` == `Current ins`. Mostly "
                                 "because tracing user of *addr* of a "
                                 "storeIns, or a tainted argument of a call\n");
        continue;
      }
      ...
      if (gv_info->Ptr &&                       // if null, the "FIELD" case
          ...
          dyn_cast<StoreInst>(inst_info->InstPtr)->getPointerOperand() ==
              gv_info->Ptr) // if store to GV
      {
        MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
        MY_DEBUG(_WARNING_LEVEL, llvm::outs()
                                     << "Don't trace target GV Assignment.\n");
        continue;
      }

      /**
       ** CONTINUE: If current instruction is before the previous one, we should
       *not record.
       **           member function comesBefore() of Instruction doesn't exist
       *in llvm-10.0.0,
      ...
      if (prev_inst_info &&
        ...
        MY_DEBUG(_WARNING_LEVEL,
                 llvm::outs() << "\" So, don't trace earlier instruction\n");
        continue;
      }

      /**
       **  CONTINUE: If the user is a continous StoreInst with same StoreAddr,
       *we don't follow it.
      ...
      if (prev_inst_info && isa<StoreInst>(prev_inst_info->InstPtr) &&
        ...
        if (prev_inst_info->InstPtr == inst_info->InstPtr)
          MY_DEBUG(_WARNING_LEVEL, llvm::outs()
                                       << "VERY WRONG. prev_inst_info->InstPtr "
                                          "== inst_info->InstPtr ");
      ...
      if (isa<StoreInst>(inst_info->InstPtr) ||
        ...
        if (std::find(visitedLoadStoreInstList.begin(),
          ...
          MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
          MY_DEBUG(_WARNING_LEVEL,
                   llvm::outs() << "Don't trace visited Load/Store Inst.\n");
          continue;
        }
      }
      ...
      /// NOTE: After filter exceptional situation, we begin to record
      /// information.
      if (prev_inst_info == nullptr) // means the first User of current GV.
      {
        gv_info->currentGVStartingFunc = inst_info->InstPtr->getFunction();
        gv_info->currentGVStartingFuncName =
            getOriginalName(inst_info->InstPtr->getFunction()->getName());
        gv_info->InstInfoList.push_back(inst_info);
        inst_info->Predecessor = nullptr;
      } else { // `(Instruction*) cur_inst` is a
               // `dyn_cast<Instruction>(cur_user)`
        if (findVisitedInstruction(inst_info, prev_inst_info))
          continue;
        if (prev_inst_info->InfluenceLevel == WEAK)
          inst_info->InfluenceLevel = WEAK;
      ...
      /// NOTE: A judgement for our handling logic, if there is any Unexpected
      /// situations here, we might need to adjust the handling logic on
      /// GEPOperator.
      ...
      for (auto it = gepo->idx_begin(), et = gepo->idx_end(); it != et; ++it) {
        ...
        if (!isa<Constant>(*it)) {
          ...
          MY_DEBUG(_ERROR_LEVEL,
                   llvm::outs()
                       << "[WARNING] Unexpected Situation: The "
                       << gepo_op_index
                       << " th Index in GEPOperator is not Constant.\n");
        }
      }

      /// NOTE: if this is the start point of structural GVs' gep instruction,
      /// check and follow.
      if (prev_inst_info ==
          nullptr) // might be the first Instruction to access structural GVs.
      {
        if (isMatchedGEPOperator(gepo, gv_info, level)) {
          MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
          MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                     << "Found start point of config: "
                                     << gv_info->NameInfo->getNameAsString()
                                     << "\n");
          ...
          MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
          MY_DEBUG(_ERROR_LEVEL,
                   llvm::outs() << "This is not a start point. [OK, PASS]\n");
        }
      } else // prev_inst_info != nullptr
      {
        MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "[ERROR] Meet GEPOperator while "
                                               "prev_inst_info != nullptr\n");
      ...
      if (prev_inst_info == nullptr) {
        ...
        MY_DEBUG(_ERROR_LEVEL, printTabs(level + 1));
        MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Src Type: " << src_type_str
                                            << "\t Dest Type: " << dest_type_str
                                            << "\n");
        ...
        if (gv_info->NameInfo->VarType == STRUCT &&
          ...
          if (pos != std::string::npos) {
            verifiedStructTypeList.push_back(
                bcop); // record these verified type cast about
                       // %struct.redisServer in Redis.
            unsigned type_offset = std::stoul(dest_type_str.substr(
                pos + src_type_str.length() + 1, dest_type_str.length()));
            MY_DEBUG(_WARNING_LEVEL, printTabs(level + 1));
            MY_DEBUG(_WARNING_LEVEL, llvm::outs() << "Current Type Offset : "
                                                  << type_offset << "\n");
      ...
      {
        MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                                   << "[ERROR] Meet BitCastOperator while "
                                      "prev_inst_info != nullptr\n");
      }
    }
      ...
      MY_DEBUG(
          _DEBUG_LEVEL, printTabs(level + 1);
          llvm::outs() << "[WARNING] Unhandled User Situation in handleUser:\n";
          printTabs(level + 2); cur_user->print(llvm::outs());
          llvm::outs() << "\n"; printTabs(level + 2);
          cur_user->getType()->print(llvm::outs()); llvm::outs() << "\n";
          string class_type = getClassType(cur_user);
          llvm::outs() << "This is a " << class_type << " Class\n\n";
...
int main2(int argc, char **argv) {
  if (argc < 3) {
    MY_DEBUG(
        _ERROR_LEVEL,
        llvm::outs() << "Expected two arguments:\n\tFirst argument - IR file "
  ...
  std::unique_ptr<llvm::Module> module =
      parseIRFile(ir_file.c_str(), Err, context);
  M = module.get();
  if (!M) {
  ...
  llvm::outs() << "Parsing DONE. [" << (endTime - startTime) / CLOCKS_PER_SEC
               << "] secs\n    This time is mandatary (LLVM library induced), "
                  "non-optimizable. "
               << "\n";
  llvm::outs() << "------------------------------------------------------------"
                  "---------\n";
  ...
  for (Module::iterator F = M->begin(), F_End = M->end(); F != F_End; ++F) {
    ...
    for (Function::iterator BB = F->begin(), BEnd = F->end(); BB != BEnd;
         ++BB) {
      ...
      for (BasicBlock::iterator inst = BB->begin(); inst != BB->end(); inst++) {
        ...
        if (GetElementPtrInst *gep_inst = dyn_cast<GetElementPtrInst>(inst)) {
          ...
          if (!gep_inst->hasIndices() || gep_inst->getNumOperands() < 3 ||
              !gep_inst->hasAllConstantIndices()) {
            ignored++;
            continue;
          }
          ...
          // some operand is not constant int, but is int, I don't know why.
          if (ignore) {
            // MY_DEBUG( _DEBUG_LEVEL, llvm::outs() <<"[NOTE] Ignore this
            // GetElementPtrInst\n"); MY_DEBUG( _DEBUG_LEVEL,
            // gep_inst->print(llvm::outs())); MY_DEBUG( _DEBUG_LEVEL,
            // llvm::outs() <<"\n"); MY_DEBUG( _DEBUG_LEVEL, srcloc.print(1));
            ignored++;
            continue;
          }
  ...
  endTime = clock();
  llvm::outs() << "Recording DONE. [" << (endTime - startTime) / CLOCKS_PER_SEC
               << "] secs"
               << "\n";

  if (recorded != GEPTypeOffsetInstList.size()) {
    llvm::outs() << "[ERROR] recorded != GEPTypeOffsetInstList.size(). Exit. "
                    "check at line: "
                 << __LINE__ << "\n";
    exit(1);
  }
  llvm::outs() << "-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   "
                  "-   -   -\n";
  llvm::outs() << "Record " << recorded << " gep_ins,  Ignore " << ignored
               << " unhandled gep_ins.\n";
  llvm::outs() << "------------------------------------------------------------"
                  "---------\n";
  ...
  for (auto I = M->global_begin(), E = M->global_end(); I != E; ++I) {
    ...
    if (!findConfigVariable(config_names, gv, gv_info_list)) {
      ...
      MY_DEBUG(_REDUNDENCY_LEVEL, llvm::outs()
                                      << "It is indeed a Config Variable\n");
    }
  }
  /// ======= FIELD ========
  // llvm::outs() << gv_info_list.size() <<" -----------------------------\n\n";
  for (uint config_names_idx = 0; config_names_idx < config_names.size();
       config_names_idx++) {
    if (config_names[config_names_idx]->VarType != FIELD)
      continue;
    vector<uint> offsets;
    offsets.push_back(
        std::atoi(config_names[config_names_idx]->FieldName[1].c_str()));
    struct GlobalVariableInfo *gv_info = new GlobalVariableInfo(
        config_names[config_names_idx], nullptr, offsets);
    gv_info_list.push_back(gv_info);
  }
  /// =====================

  /// NOTE: Record config variables found in dbg info.
  string var_found_path =
      ir_file.substr(0, ir_file.find_last_of(".")) + "-found.txt";
  fstream var_found_f(var_found_path.c_str(), fstream::out | ios_base::trunc);
  if (!var_found_f) {
    MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                               << "open " << var_found_path << " FAILED!\n");
    return -1;
  }
  ...
  MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "\n\n\n*******************************"
                                         "***************************\n\n\n\n");
  MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "Found " << gv_info_list.size()
                                      << " GlobalVariables in total.\n");
  MY_DEBUG(
      _DEBUG_LEVEL,
      for (auto i = gv_info_list.begin(), e = gv_info_list.end(); i != e; i++) {
        struct GlobalVariableInfo *gv_info = *i;
        ...
        if (gv_info->NameInfo->VarType != SINGLE &&
            gv_info->NameInfo->VarType != FIELD)
          MY_DEBUG(_DEBUG_LEVEL,
                   llvm::outs()
                       << "GV Type: " << gv_info->GlobalVariableType << "\n";);
      })
  MY_DEBUG(_DEBUG_LEVEL, llvm::outs() << "\n\n\n*******************************"
                                         "***************************\n\n\n\n");
  ...
  endTime = clock();
  llvm::outs() << "\nTainting DONE. [" << (endTime - startTime) / CLOCKS_PER_SEC
               << "] secs"
               << "\n";
  llvm::outs() << "------------------------------------------------------------"
                  "---------\n";
  string output_file =
      ir_file.substr(0, ir_file.find_last_of(".")) + "-records.dat";
  fstream existing_f(output_file.c_str(), fstream::out | ios_base::trunc);
  if (!existing_f) {
    MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Open output file " << output_file
                                        << " FAILED!\n");
    return -1;
  }
  existing_f.close();
  for (auto i = gv_info_list.begin(), e = gv_info_list.end(); i != e; i++) {
    ...
    if (!gv_info->writeToFile(output_file)) {
      MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Write to output file "
                                          << output_file << " FAILED!\n");
      break;
    }
  }
   ...
   *******************************************************************/
  string keyinfo_file =
      ir_file.substr(0, ir_file.find_last_of(".")) + "-keyinfo.txt";
  fstream keyinfo_f(keyinfo_file.c_str(), fstream::out | ios_base::trunc);
  if (!keyinfo_f) {
    MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Open statistics file "
                                        << keyinfo_file << " FAILED!\n");
    return -1;
  }
  keyinfo_f.close();
  for (auto i = gv_info_list.begin(); i != gv_info_list.end(); i++) {
    ...
    if (!gv_info->writeKeyInfo(keyinfo_file)) {
      MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Write to statistics file "
                                          << keyinfo_file << " FAILED!\n");
      MY_DEBUG(_ERROR_LEVEL,
               llvm::outs() << "Current GlobalVariable is "
                            << gv_info->NameInfo->getNameAsString() << "\n");
      break;
    }
  }
...
int buildModule(std::unique_ptr<llvm::Module> &module, LLVMContext &context,
                SMDiagnostic &Err, string ir_file) {
  if (ir_file.substr(ir_file.length() - 3, 3) != ".bc" &&
      ir_file.substr(ir_file.length() - 3, 3) != ".ll") {
    MY_DEBUG(_ERROR_LEVEL, llvm::outs()
                               << "First argument format WRONG!\n\tExpect for "
...
std::vector<struct GlobalVariableInfo *>
getGlobalVariableInfo(std::unique_ptr<llvm::Module> &module,
                      string var_string) {
  M = module.get();
  if (!M) {
    MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Parese module error!\n");
  ...
  struct ConfigVariableNameInfo *config_name =
      readConfigVariableNameByOne(var_string);
  std::vector<struct ConfigVariableNameInfo *> config_names;
  config_names.push_back(config_name);
...
std::vector<struct GlobalVariableInfo *> getGlobalVariableInfo(
    std::unique_ptr<llvm::Module> &module,
    std::vector<struct ConfigVariableNameInfo *> config_names) {
  M = module.get();
  if (!M) {
  ...
  MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "\n\n\n*******************************"
                                         "***************************\n\n\n\n");
  MY_DEBUG(_ERROR_LEVEL, llvm::outs() << "Found " << gv_info_list.size()
                                      << " GlobalVariables in total.\n");
  MY_DEBUG(
      _DEBUG_LEVEL,
      for (auto i = gv_info_list.begin(), e = gv_info_list.end(); i != e; i++) {
...
int startAnalysis(std::vector<struct GlobalVariableInfo *> gv_info_list,
                  bool isAnalysisImplicitData, bool isAnalysisImplicitControl) {
  clock_t startTime = clock();
  visitedStructGVCases.clear();
  for (unsigned gv_cnt = 0; gv_cnt < gv_info_list.size(); gv_cnt++) {
    //  gv_info_list    ~~  std::vector<GlobalVariableInfo*>
...
int main3(int argc, char **argv) {
  ...
  std::vector<struct GlobalVariableInfo *> gvlist =
      getGlobalVariableInfo(module, var_string);
