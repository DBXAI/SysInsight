...
class LLM_GEN_SM:
    ...
    async def _predict(self, all_prompt_templates, query_examples):
        ...
        print("hzytime:", time_taken)
        return mean_probs, success_rate, tot_cost, tot_tokens, time_taken
    
    async def _evaluate_candidate_points(self, observed_configs, observed_fvals, candidate_configs, config = None):
        ...
        all_prompt_templates, query_examples = gen_prompt_tempates(self.task_context, observed_configs, observed_fvals, candidate_configs, 
                                                                   self.lower_is_better, self.top_pct, n_prompts=self.n_templates, config = config)
    ...
    def select_query_point(self, observed_configs, observed_fvals, candidate_configs,  config = None, return_raw_preds=False):
        ...
        pred_probs, cost, time_taken = asyncio.run(self._evaluate_candidate_points(observed_configs, observed_fvals, candidate_configs, config=config))
