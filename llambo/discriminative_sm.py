...
class LLM_DIS_SM:
    ...
    async def _generate_concurrently(self, few_shot_templates, query_examples):
        ...
        for template in few_shot_templates:
            template.prefix = template.prefix.replace("<hzt<", "{").replace(">hzt>", "}")
    ...
    async def _evaluate_candidate_points(self, observed_configs, observed_fvals, candidate_configs, 
                                         use_context='full_context', use_feature_semantics=True, return_ei=False, config = None):
        ...
        all_prompt_templates, query_examples = gen_prompt_tempates(self.task_context, observed_configs, observed_fvals, candidate_configs, 
                                                                    ...
                                                                    shuffle_features=self.shuffle_features, apply_warping=self.apply_warping, promptlib=config)
    ...
    def select_query_point(self, observed_configs, observed_fvals, candidate_configs,  config = None):
        ...
        y_mean, y_std, cost, time_taken = asyncio.run(self._evaluate_candidate_points(observed_configs, observed_fvals, candidate_configs, config = config))
        if self.lower_is_better:
            best_fval = np.min(observed_fvals.to_numpy())
            delta = -1*(y_mean - best_fval)
        else:
