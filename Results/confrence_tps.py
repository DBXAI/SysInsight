...
def extract_lat_sys_eye(data):
    return [entry["external_metrics"]["tps"] for entry in data if "external_metrics" in entry and "tps" in entry["external_metrics"]]

# 文件路径
file_sys_eye_1 = "/root/sysinsight-main/rule_collect_results_sysbench-notbetter.json"
file_sys_eye_2 = "/root/sysinsight-main/rule_collect_results_sysbench-better.json"
...
# **计算累积最小值**
min_lat_sys_eye_1 = np.maximum.accumulate(lat_sys_eye_1)
min_lat_sys_eye_2 = np.maximum.accumulate(lat_sys_eye_2)
...
plt.title("TPS Convergence Comparison (sys-eye)", fontsize=16, fontweight="bold")
plt.legend(fontsize=10, loc="lower right")
plt.grid(True, linestyle="--", alpha=0.6)

# **显示图像**
plt.savefig("sys_eye_prompt_comparison.png", dpi=300, bbox_inches="tight")
plt.show()
